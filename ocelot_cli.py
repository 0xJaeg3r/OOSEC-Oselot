...
Example tasks:
  ...
  • Generate an OSINT report on a target domain
...
def print_help():
    ...
    help_text = """
╔═══════════════════════════════════════════════════════════════╗
║  Oselot OSINT Agent - Help & Commands                   ║
╚═══════════════════════════════════════════════════════════════╝
...
COMMANDS:
  ...
  /quit      - Exit the Ocelot CLI

AVAILABLE MODELS:
  OpenAI:
    • gpt-5.3-codex        - GPT-5.3 Codex (Agentic Coding, Latest)
    • gpt-5.2              - GPT-5.2 (Flagship, Professional Knowledge)
    • gpt-5                - GPT-5 (Base Model)
    • o3                   - O3 Reasoning Model (Full)
    • o3-mini              - O3 Reasoning Model (Mini)

  Anthropic:
    • claude-opus-4-6      - Claude Opus 4.6 (Most Capable, Agent Teams)
    • claude-sonnet-5      - Claude Sonnet 5 "Fennec" (Fast, 82% SWE-Bench)
    • claude-opus-4-5      - Claude Opus 4.5

  Google:
    • gemini-3-pro         - Gemini 3 Pro (Most Intelligent)
    • gemini-3-flash       - Gemini 3 Flash (Pro-level at Flash Speed)
    • gemini-2.5-pro       - Gemini 2.5 Pro (1M Context)
    • gemini-2.5-flash     - Gemini 2.5 Flash (Lightning Fast)
...
CAPABILITIES:
  OSINT & Recon Tools:
    - ASN mapping (org/domain/IP to CIDR ranges)
    - Subdomain enumeration (bbot)
    - Web fingerprinting & liveness checking (httpx)
    - Web search (Tavily)
    - File operations (read, write, search)
    - System commands (echo, pipe, ls, cat, find)

  Team Coordination:
    - Hierarchical agent delegation
    - Multi-specialist collaboration
    - Comprehensive OSINT assessments

EXAMPLE QUERIES:
  • Perform subdomain enumeration on example.com
  • Find ASN and IP ranges for an organization
  • Detect technologies running on discovered subdomains
  • Generate an OSINT report for a target domain
...
def get_model_input():
    ...
    print("\n  Google:")
    print("    7. gemini-3-pro         (Most Intelligent)")
    print("    8. gemini-3-flash       (Fast)")
    print("    9. gemini-2.5-pro       (1M Context)")
    print("\n    0. Enter custom model ID")

    choice = input("\n📋 Select (0-9) or press Enter for default: ").strip()
...
Active Agents: 4 OSINT Specialists
Available Tools:
  • ASN Mapping (asnmap)
  • Subdomain Enumeration (bbot)
  • Web Fingerprinting & Liveness (httpx)
  • Web Search (Tavily)
  • File Operations (read, write, search)
  • System Commands (echo, pipe, ls, cat, find)"""
...
def main():
    """Main CLI loop"""
    parser = argparse.ArgumentParser(description='Ocelot OSINT Agent CLI')
    ...
    try:
        ...
        print(f"\n✗ Failed to initialize Ocelot agent: {e}")
